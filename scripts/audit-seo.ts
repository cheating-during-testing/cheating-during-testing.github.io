...
function isGoogleVerificationFile(path: string) {
  return /^\/google[a-z0-9_-]+\.html$/i.test(routeForFile(path));
}
...
const files = collectHtml(output).filter((path) => {
  ...
  return route !== "/404/"
    && route !== "/_not-found/"
    && route !== "/404.html"
    && route !== "/_not-found.html"
    && !isGoogleVerificationFile(path);
});
...
const pageTypeByRoute = new Map(enabledPages.map((page) => [`/${page.slug}/`, page.pageType]));
...
for (const file of files) {
  ...
  const expectedRobot = siteConfig.readyForLaunch && pageTypeByRoute.get(route) !== "legal" ? "index" : "noindex";
